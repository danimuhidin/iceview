...
@section('title', 'Edit Garansi Admin | Ice View Indonesia')
...
@section('content')
    <section class="rounded-2xl border border-slate-700/60 bg-[#0b1222]/85 p-6 sm:p-8">
        <div class="mb-6">
            ...
            <p class="mt-1 text-sm text-slate-300">Admin dapat mengubah semua field utama garansi.</p>
        </div>

        <form method="POST" action="{{ route('admin.warranties.update', $warranty) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="car_type" value="{{ old('car_type', $warranty->car_type) }}"
                        class="w-full rounded-md border border-slate-500/40 bg-[#111d33] px-3 py-2 text-sm text-slate-100 outline-none focus:border-[#00F0FF]"
                        required>
                </div>
                <div>
                    <label class="mb-2 block text-sm text-slate-200">Nomor Mesin</label>
                    <input type="text" name="engine_number" value="{{ old('engine_number', $warranty->engine_number) }}"
                        class="w-full rounded-md border border-slate-500/40 bg-[#111d33] px-3 py-2 text-sm text-slate-100 outline-none focus:border-[#00F0FF]"
                        required>
                </div>
            </div>
