...
@section('content')
    <section class="rounded-2xl border border-slate-700/60 bg-[#0b1222]/85 p-6 sm:p-8">
        ...
        <p class="mb-8 text-sm text-slate-300 sm:text-base">Pantau data garansi, status claim, dan update terbaru dari
            Iceview.</p>

        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            <div class="rounded-xl border border-slate-600/50 bg-[#111d33] p-4">
                <p class="text-sm text-slate-300">Jumlah Garansi</p>
            ...
            <div class="rounded-xl border border-slate-600/50 bg-[#111d33] p-4">
                <p class="text-sm text-slate-300">Garansi Pending</p>
                <p class="mt-2 text-2xl font-bold text-white">{{ number_format($stats['pending_claim']) }}</p>
            </div>
            <div class="rounded-xl border border-slate-600/50 bg-[#111d33] p-4">
                <p class="text-sm text-slate-300">Item Active</p>
                <p class="mt-2 text-2xl font-bold text-white">{{ number_format($stats['active_item']) }}</p>
            </div>
            <div class="rounded-xl border border-slate-600/50 bg-[#111d33] p-4">
                <p class="text-sm text-slate-300">Item Claimed</p>
                <p class="mt-2 text-2xl font-bold text-white">{{ number_format($stats['claimed_item']) }}</p>
            </div>
        </div>
    </section>
@endsection
